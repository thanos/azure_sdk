defmodule ExAzure.Error do
  @moduledoc """
  Standardized, pattern-match friendly error struct for all ExAzure operations.

  All public APIs return `{:ok, result}` or `{:error, %ExAzure.Error{}}`.
  """
  ...
  def from_response(service, status, headers, body) do
    parsed = ExAzure.Core.Xml.Error.parse(body)
