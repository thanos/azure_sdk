defmodule ExAzure.Pipeline.SharedKey do
  @moduledoc false
  ...
  """
  @spec apply(Request.t(), ExAzure.Identity.SharedKeyCredential.t()) :: Request.t()
  def apply(%Request{} = request, %ExAzure.Identity.SharedKeyCredential{} = credential) do
    request =
      request
      |> ensure_ms_headers()
    ...
    |> tap(fn _ ->
      ExAzure.Core.Telemetry.emit_sign(%{
        scheme: :shared_key,
